/*
 *
 *  HVDIMM character driver for MMLS.
 *
 *  (C) 2015 Netlist, Inc.
 *
...
typedef struct privatedata {
	...
	unsigned long mmls_nsectors;	/* mmls num of sectors */
} hv_cdev_private;
...
#if HW_INSTALLED	/* NOTE: Undef this if hv hardware actually installed. If not, use ramdisk */
 ...
 * mmls_init()
 *
 * This function returns mmls size and phys addr from hv cmd
 * driver.
 *
...
static void __exit hv_cdev_exit(void)
{
	...
	/* Free up mmls memory. Need mod if need to support >1 mmls dev */
	/* This func is in cmd drv */
	mmls_iomem_release();
...
MODULE_AUTHOR("S. Gosali <sgosali@netlist.com>");
MODULE_DESCRIPTION("Netlist HVDIMM MMLS char driver");
